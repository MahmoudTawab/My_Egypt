...
open class FPNTextField: UITextField {
    ...
    @objc open var flagButtonSize: CGSize = CGSize(width: ControlWidth(30), height: ControlWidth(30)) {
        didSet {
            layoutIfNeeded()
        }
    }
...
//        let width = flagButtonSize.width + getWidth(text: phoneCodeTextField.text ?? "  ") + ControlWidth(10)
    ...
    @objc open var hasPhoneNumberExample: Bool = false {
        didSet {
    ...
    private func setup() {
        ...
        phoneCodeTextField.isEnabled = false
        self.layer.borderWidth = ControlWidth(2)
        self.font = UIFont(name: "Nexa-Regular", size: ControlWidth(16))
        phoneCodeTextField.font = UIFont(name: "Nexa-Bold", size: ControlWidth(14))
...
//        setupLeftView()
        ...
        title.font = UIFont(name: "Nexa-Bold", size: ControlWidth(13))
          
        self.addSubview(title)
        title.translatesAutoresizingMaskIntoConstraints = false
        title.bottomAnchor.constraint(equalTo: self.topAnchor).isActive = true
        ...
        title.heightAnchor.constraint(equalToConstant: ControlWidth(30)).isActive = true
        
        ///
        PhoneLabel.alpha = 0
        PhoneLabel.textColor = #colorLiteral(red: 0.9254902005, green: 0.2352941185, blue: 0.1019607857, alpha: 1)
        PhoneLabel.backgroundColor = .clear
        PhoneLabel.translatesAutoresizingMaskIntoConstraints = false
        PhoneLabel.font = UIFont(name: "Nexa-Regular", size: ControlWidth(12))
        addSubview(PhoneLabel)
        
        PhoneLabel.leadingAnchor.constraint(equalTo: self.leadingAnchor).isActive = true
        PhoneLabel.trailingAnchor.constraint(equalTo: self.trailingAnchor).isActive = true
        PhoneLabel.topAnchor.constraint(equalTo: self.bottomAnchor , constant: ControlWidth(5)).isActive = true
        PhoneLabel.heightAnchor.constraint(equalToConstant: ControlWidth(20)).isActive = true
        
        self.leftView = UIView(frame: CGRect(x: 0, y: 0, width: flagButtonSize.width + ControlX(16) , height: flagButtonSize.height))
        self.leftViewMode = .always
        
        self.leftView?.isUserInteractionEnabled = true
        self.leftView?.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(displayCountries)))
    }
    
    
    var isFloatingTitleHidden = true
    open var TitleHidden : Bool = true
    ...
    private func setupFlagButton() {
        ...
        flagButton.centerYAnchor.constraint(equalTo: self.centerYAnchor).isActive = true
        ...
        flagButton.leadingAnchor.constraint(equalTo: self.leadingAnchor, constant: ControlX(10)).isActive = true
    }
...
//        phoneCodeTextField.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(displayCountries)))
//    }
...
//
//        NSLayoutConstraint(item: flagButton, attribute: .leading, relatedBy: .equal, toItem: leftView, attribute: .leading, multiplier: 1, constant: ControlX(2)).isActive = true
...
//        NSLayoutConstraint(item: phoneCodeTextField, attribute: .top, relatedBy: .equal, toItem: leftView, attribute: .top, multiplier: 1, constant:  ControlX(1)).isActive = true
...
//
//        flagButton.imageView?.clipsToBounds = true
//        flagButton.imageView?.layer.cornerRadius = ControlX(4)
//
//        leftView?.isUserInteractionEnabled = true
//        leftView?.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(displayCountries)))
//    }
//
...
//        let newRect: CGRect = CGRect(x: bounds.minX + ControlX(2), y: bounds.minY, width: width, height: height)
//        return newRect
//    }
...
//        let phoneCode = selectedCountry?.phoneCode,
        if  let number = text {
//            var cleanedPhoneNumber = clean(string: "\(phoneCode) \(number)")

            if let validPhoneNumber = getValidNumber(phoneNumber: number) {
...
//                if let inputString = formatter?.inputString(cleanedPhoneNumber) {
                    text = number
//                remove(dialCode: phoneCode, in: inputString)
//                }
...
//                    if let inputString = formatter?.inputString(cleanedPhoneNumber) {
                        text = number
//                    remove(dialCode: dialCode, in: inputString)
//                    }
//                }
    ...
    private func updateUI() {
        ...
        flagButton.setBackgroundImage(selectedCountry?.flag?.withInset(UIEdgeInsets(top: 3, left: 3, bottom: 3, right: 3)), for: .normal)
