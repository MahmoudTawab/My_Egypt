...
class SSRadioButton: UIButton {
     ...
     */
    @IBInspectable var circleColor: UIColor = #colorLiteral(red: 0.9608150125, green: 0.7241352201, blue: 0.1713911593, alpha: 1) {
        didSet {
     ...
     */
    @IBInspectable var strokeColor: UIColor = #colorLiteral(red: 0, green: 0, blue: 0, alpha: 1) {
        didSet {
            circleLayer.strokeColor = strokeColor.cgColor
            self.toggleButon()
        }
     ...
     */
    @IBInspectable var circleRadius: CGFloat = ControlWidth(8)
    ...
    fileprivate func initialize() {
        circleLayer.frame = CGRect(x: circleRadius * 4, y: circleRadius * 4, width: bounds.width - (circleRadius * 8), height: bounds.height - (circleRadius * 8))
        circleLayer.lineWidth = ControlWidth(2)
        circleLayer.fillColor = UIColor.clear.cgColor
        circleLayer.strokeColor = strokeColor.cgColor
        layer.addSublayer(circleLayer)
        fillCircleLayer.frame = bounds
        fillCircleLayer.lineWidth = ControlWidth(3)
        fillCircleLayer.fillColor = UIColor.clear.cgColor
        fillCircleLayer.strokeColor = UIColor.clear.cgColor
        layer.addSublayer(fillCircleLayer)
        self.titleEdgeInsets = UIEdgeInsets(top: 0, left: (1.5*circleRadius + 1.5*circleLayer.lineWidth) + ControlWidth(10), bottom: 0, right: 0)
        self.toggleButon()
    }
    /**
    ...
    fileprivate func fillCirclePath() -> UIBezierPath {
        return UIBezierPath(ovalIn: circleFrame().insetBy(dx: 2.2, dy: 2.2))
    }
