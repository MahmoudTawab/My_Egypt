...
class EventsDetailsVC: ViewController ,UITableViewDelegate , UITableViewDataSource ,EventsDetailsDelegate ,UIViewControllerTransitioningDelegate {
    
    var EventId : Int?
    var ReadMore = false
    var DataEventDetails : EventDetails?
  ...
  fileprivate func SetUpItems() {
      ...
      HeaderView.addSubview(EventsImage)
      HeaderView.addSubview(EventsBackground2)
        
      EventsBackground1.frame = CGRect(x: 0, y: ControlY(10), width: HeaderView.frame.width, height: HeaderView.frame.height - ControlY(50))
      EventsImage.frame = CGRect(x: 0, y: ControlY(110), width: HeaderView.frame.width, height: HeaderView.frame.height - ControlY(150))
      EventsBackground2.frame = CGRect(x: 0, y: ControlY(10), width: HeaderView.frame.width, height: HeaderView.frame.height - ControlY(50))
      EventsImage.layer.cornerRadius = EventsImage.frame.width / 2
      ...
      SiaInsightsButton.frame = CGRect(x: EventsImage.frame.width - ControlX(130), y: EventsImage.frame.minY + ControlX(50), width: ControlWidth(100), height: ControlWidth(100))

      HeaderView.addSubview(RatingAndFavorites)
      RatingAndFavorites.frame = CGRect(x: ControlX(15), y: EventsBackground2.frame.maxY , width: view.frame.width - ControlX(30) , height: ControlWidth(50))
    ...
    Map.Textsearch = DataEventDetails?.AllDetails?.address ?? ""
    Map.modalPresentationStyle = .fullScreen
    present(Map, animated: true, completion: nil)
    }
    
    lazy var EventsDetailsStack : UIStackView = {
    ...
    lazy var TopStack : UIStackView = {
        let Stack = UIStackView(arrangedSubviews: [ViewDismiss,EventsDetailsStack])
    ...
    lazy var TableView : UITableView = {
        ...
        tv.register(EventsDetailsCell.self, forCellReuseIdentifier: CellId)
        return tv
    }()
    ...
    lazy var EventsImage : ImageViewGradient = {
    ...
    if let image = EventsImage.image {
    let imageInfo = GSImageInfo(image: image, imageMode: .aspectFit)
    let transitionInfo = GSTransitionInfo(fromView: EventsImage)
    ...
    @objc func SiaInsightsAction() {
        if DataEventDetails?.AllDetails?.insights == true {
            ...
            SiaInsightsView.MessageTV.text = DataEventDetails?.AllDetails?.insightsContent ?? ""
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: CellId, for: indexPath) as! EventsDetailsCell
        cell.Event = DataEventDetails

        cell.Delegate = self
        cell.EventsDetails = self
        cell.selectionStyle = .none
        cell.backgroundColor = .white
        
        if DataEventDetails?.AllDetails?.description?.TextHeight(cell.frame.width , font: UIFont.systemFont(ofSize: ControlWidth(16)), Spacing: ControlWidth(8)) ?? 0 > ControlWidth(50) {
    ...
    let PlacesDetailsInstance = EventsDetailsInstanceVC()
    PlacesDetailsInstance.EventInstanceId = id
    ...
    let parameters:[String:Any] = ["PlaceId":  0,
                                   "EventId": EventId,
    ...
    func GetDataEventDetails() {
    self.view.subviews.forEach { View in View.alpha = 0}
    guard let url = defaults.string(forKey: "API") else{
    LodBaseUrl()
    self.SetUpIsError(true) {
    self.GetDataEventDetails()}
    return
    }

    let api = "\(url + GetEventDetails)"
    let EventId = EventId ?? 0
    let token = defaults.string(forKey: "jwt") ?? ""
    ...
    self.SetData(EventDetails(dictionary: dictionary))
    } array: { _ in
    } Err: { error in
    self.SetUpIsError(true) {
    self.GetDataEventDetails()
    ...
    func SetData(_ Data: EventDetails)  {
        DataEventDetails = Data
        NameLabel.text = Data.AllDetails?.eventName ?? ""
        ...
        EventsImage.sd_setImage(with: URL(string: Data.AllDetails?.image ?? "")
...
class EventsDetailsInstanceVC: EventsDetailsVC {
    
    var EventInstanceId : Int?
    override func viewDidLoad() {
        super.viewDidLoad()
